...
#Make a palette
  pal_era5 <- colorNumeric(palette = colorRamp(c("white", "magenta"), interpolate = "spline"),
                      domain = unique(values(mean_era5_wind_speed)),na.color = NA)
...
leaflet() %>%
  ...
  addRasterImage(x = mean_era5_wind_speed,
                 group = "Wind",
                 colors = pal_era5,
                 opacity = 1) %>%
  ...
  leaflet::addLegend(position = "bottomright",
            pal = pal_era5,
            values = min(na.omit(values(mean_era5_wind_speed))):max(na.omit(values(mean_era5_wind_speed))),
            opacity = 1,
            title = "Mean\nWind\nSpeed\n(m/s)") %>%
    addLayersControl(
