...
leaflet() %>%
  ...
  leaflet::addLegend(position = "bottomright",
    ...
    leaflet::addLegend(position = "bottomleft",
            pal = pal_era5,
            values = min(unique(values(median_era5_wind_speed))):max(unique(values(median_era5_wind_speed))),
            opacity = 1,
            title = "Median\nWind\nSpeed",) %>%
    addLayersControl(
    baseGroups = c("World Imagery","NatGeo"),
    overlayGroups = c("Flights","Requests","Clouds","Wind"),
    options = layersControlOptions(collapsed = FALSE),
    position = "topright") %>%
  hideGroup(c("Requests","Wind"))
